...
sudo apt-get install -y curl htop net-tools jq tmux python3 python3-venv wget gnupg2 software-properties-common unzip

# Crear directorios de logs y permisos
...
sudo mkdir -p /etc/loki /var/loki/index /var/loki/cache /var/loki/chunks
...
# Configurar entorno virtual para Flask
python3 -m venv ~/opensoc_env
source ~/opensoc_env/bin/activate
pip install --upgrade pip
pip install flask

# Crear menú web en Flask
...
echo "Activa el entorno virtual con: source ~/opensoc_env/bin/activate"
echo "Inicia el menú web con: python3 ~/opensoc_logserver_web.py"
